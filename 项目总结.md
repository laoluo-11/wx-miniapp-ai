...
| 项目 | 内容 |
|------|------|
| LaTeX 清理 | 删除消息时正则提取公式，MD5 匹配删除缓存 SVG |
| 用户隔离 | 关 boot-md + session-memory hook；system prompt 注入昵称 |
| 语音测评 | 统一 50 词段落；预置默认句即时显示；切引导语 |
| 服务器 | uvicorn 2 workers + keepalive 65s |
...
### 域名与部署
- 正式域名切换到 `yyzhilingweilai.com`（已备案）
- HTTPS 证书：Let's Encrypt，自动续期
- Nginx 反向代理 → uvicorn:8000（4 workers）
- 阿里云安全组已开放 80/443 端口

### 管理后台（新增）
- 访问地址：`https://yyzhilingweilai.com/admin`
- 功能：用户管理、对话查看、评测/记忆/文件管理、数据删除
- 后端：`server/app/routers/admin.py`

### 会话标题优化
- 创建时显示时间戳，发消息后 AI 自动生成语义化标题
- 修复前端预创建会话导致的标题不更新问题

### 其它
- 手机号绑定功能（微信 getPhoneNumber 解密）
- 语音测评增加"换一句"按钮
- 修复 message.py fetchall 类型错误
- 旧域名 luois-james.xyz 文件清理兼容

---

## 2026-07-28 — VIP会员系统 & 用量监控完成

### 核心功能
- 普通/会员双身份，到期自动降级
- 四种操作（对话/评测/对练/上传）分开限额
- 个人中心实时用量进度条
- 管理后台全功能 VIP 管理
